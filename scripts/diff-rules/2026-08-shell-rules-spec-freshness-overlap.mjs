...
//     `copilot-requests`) disappears on replay.
...
function isNewlyRecognizedPermScope(finding) {
  const message = finding?.message ?? "";
  return (
    finding?.rule === "permissions-syntax" &&
    NEWLY_RECOGNIZED_PERM_SCOPES.some((s) => message.includes(`unknown permission scope \`${s}\``))
  );
}
