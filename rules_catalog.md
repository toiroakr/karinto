...
| `archived-uses` | [`zizmor:archived-uses`](https://docs.zizmor.sh/audits/#archived-uses) | warning | Implemented | Three additive sources: the engine's hardcoded baseline (`actions/setup-ruby`, fires offline), a KV-cached baseline that `cf/index.js`'s daily cron maintains (candidates mined from recent capture traffic, confirmed via the GitHub API `archived` flag), and the per-request `archived` parameter CSV. Comparison is case-insensitive and operates on the bare `owner/repo` (subpath stripped). |
