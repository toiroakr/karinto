...
83 catalogued rules: **72 implemented**, **4 planned**, **7 not planned**,
...
| `expression-type-mismatch` | [`actionlint:expression-type-mismatch`](https://github.com/rhysd/actionlint/blob/main/docs/checks.md) | error | Implemented | Literal-level inference: parses each `${{ … }}` it can (unsupported constructs are skipped) and flags arithmetic operators (`+`/`*`/`/`/`%` — the expression language has no arithmetic; `*` here means the infix use, not the `.*`/`[*]` object filter) applied to known non-number operand types (literals and known function returns). Context-derived operand types stay with the per-context rules below; upstream additionally types every context property and function signature. |
...
| `expression-steps-type` | [`actionlint:steps-id-type`](https://github.com/rhysd/actionlint/blob/main/docs/checks.md) | error | Implemented | Tracks declared step `id`s per job (composite actions included) honouring step order: `steps.<id>` references to unknown or not-yet-run steps are errors, as are unknown fields on `steps.<id>` and `steps.<id>.outputs` used as a scalar. Output *names* stay untyped (set at runtime via `$GITHUB_OUTPUT`); upstream narrows them through action metadata for `uses:` steps. Dynamic `id:` values disable the check for that scope. Bare `if:` conditions (no `${{ … }}` delimiters) are type-checked too, matching GitHub's evaluation rules. |
| `expression-matrix-type` | [`actionlint:matrix-type`](https://github.com/rhysd/actionlint/blob/main/docs/checks.md) | error | Implemented | Flags `matrix.<key>` references not declared as a `strategy.matrix` dimension or introduced by an `include:` entry. Expression-built matrices (`matrix: ${{ … }}` or expression `include:`) disable the check. Dimension *value* types are not modelled (upstream types them from the value lists), and `matrix.*` outside any `strategy:` is left to `context-availability`-level checks. Bare `if:` conditions (no `${{ … }}` delimiters) are type-checked too, matching GitHub's evaluation rules. |
| `expression-needs-type` | [`actionlint:needs-type`](https://github.com/rhysd/actionlint/blob/main/docs/checks.md) | error | Implemented | Types `needs` from the job dependency graph: only direct dependencies are members (transitive references are errors, matching upstream), `needs.<job>.outputs` is strict over the dependency's declared `outputs:` keys, plus `result`. Reusable-workflow dependencies (`uses:`) keep loose outputs since the called workflow is unseen. Dynamic `needs:` values disable the check. Bare `if:` conditions (no `${{ … }}` delimiters) are type-checked too, matching GitHub's evaluation rules. |
...
| `shellcheck` | [`actionlint:shellcheck-on-run`](https://github.com/rhysd/actionlint/blob/main/docs/checks.md) | warning | Not planned | A 2026-08-10 investigation found WASM (`wasilibs/go-shellcheck`) partially invalidates the "can't ship a native binary" rationale, but it was evaluated and rejected: GPL-3.0 (license-hygiene overhead for this Apache-2.0 repo), an unproven WASI shim under workerd, and a GHC-runtime CPU cost that doesn't fit the free plan's 10ms budget. `#113` adds targeted tree-sitter-bash rules instead (`shell-quote-safety`, `shell-undefined-var` below, and the tree-sitter-based `github-env` / `unpinned-tools` / `unredacted-secrets` / `use-trusted-publishing`) — not a shellcheck substitute or a claim of equivalent coverage. |
| `pyflakes` | [`actionlint:pyflakes-on-python-run`](https://github.com/rhysd/actionlint/blob/main/docs/checks.md) | warning | Not planned | Out of scope entirely, unrelated to the shellcheck WASM question above. GitHub-hosted runners don't preinstall pyflakes either, so actionlint's own pyflakes rule is a silent no-op in most CI anyway. |
| `script-injection` | [`actionlint:script-injection`](https://github.com/rhysd/actionlint/blob/main/docs/checks.md) | error | Not planned | Same vulnerability surface as zizmor's `template-injection`, which karinto ships as the [`template-injection`](#supply-chain--security-zizmor) rule. Verified against actionlint's `examples/untrusted_input.yaml`, `err/nested_untrusted_input.yaml`, `err/github_script_untrusted_input.yaml` — every actionlint-flagged sink is also caught by zizmor's audit, which additionally backstops Info-level findings on every `${{ … }}`. Finding granularity differs (actionlint emits one per untrusted subexpression; zizmor emits one per template expression), but no vulnerable site is missed. Actionlint testdata fixtures where karinto fires `template-injection` while actionlint stays silent are tracked via `expect: [fires:template-injection]` in `scripts/upstream-parity/allowlist.json` so regressions surface as hard divergences. |
...
| `archived-uses` | [`zizmor:archived-uses`](https://docs.zizmor.sh/audits/#archived-uses) | warning | Implemented | Three additive sources: the engine's hardcoded baseline (`actions/setup-ruby`, fires offline), a maintained baseline kept in committed `cf/archived.json` (bundled into the Worker) and mirrored to KV for live reads — a daily CI job (`refresh-archived.yml`) drains the D1 worklist the request path fills, confirms each via the GitHub API `archived` flag, re-verifies the existing set every run, and writes both KV and a PR for `cf/archived.json` — and the per-request `archived` parameter CSV. Comparison is case-insensitive and operates on the bare `owner/repo` (subpath stripped). |
...
| `github-env` | [`zizmor:github-env`](https://docs.zizmor.sh/audits/#github-env) | error | Implemented | Fires on non-literal writes to `$GITHUB_ENV` / `$GITHUB_PATH` — static writes are silent. Workflow steps require a dangerous trigger (`pull_request_target` / `workflow_run`); composite-action steps are checked unconditionally. `#113`: parses `run:` with tree-sitter-bash (matching upstream's approach), replacing the previous per-line regex heuristic — the regex version couldn't see inside a `cat <<EOF >> "$GITHUB_ENV"` heredoc body at all (false negative), which the AST version catches, including correctly honouring a quoted heredoc delimiter (`<<'EOF'`) disabling expansion. The tree-sitter-bash parse only runs when the step's resolved shell is bash/sh (step `shell:` > job/workflow `defaults.run.shell` > runner default); a pwsh (`$env:GITHUB_ENV`) or cmd (`%GITHUB_ENV%`) step is skipped entirely, not analyzed — narrower than the old regex heuristic, which nominally recognized all three syntaxes but only via naive substring matching. |
...
| `impostor-commit` | [`zizmor:impostor-commit`](https://docs.zizmor.sh/audits/#impostor-commit) | error | Not planned | Needs live GitHub API resolution of per-`repo@sha` membership, which the edge linter does not perform. **Delegated to the companion action** (`companion-action/`): karinto surfaces the SHA-pinned refs in the response's `online_audit_candidates`, and the companion checks membership and reports directly. See [docs/action-context.md](action-context.md). |
...
| `known-vulnerable-actions` | [`zizmor:known-vulnerable-actions`](https://docs.zizmor.sh/audits/#known-vulnerable-actions) | error | Implemented | Decided **solely** by the online advisory path (OSV.dev via `osv=1` / the companion action) — the same advisory-DB mechanism zizmor uses. No hardcoded fallback list: a hand-maintained one cannot track GHSA's per-advisory version ranges and false-flagged fixed releases (e.g. `tj-actions/changed-files@v47`). Without `osv=1` the rule does not fire. |
...
| `obfuscation` | [`zizmor:obfuscation`](https://docs.zizmor.sh/audits/#obfuscation) | info | Implemented | Detects path-segment tricks in `uses:` (empty / `.` / `..` / percent-encoded refs), constant-reducible `${{ ... }}` expressions, computed index access (`foo[bar.baz]`), and expression-based `with:` mappings that prevent static analysis. Steps whose `if:` is statically false (`if: false` or `if: ${{ false }}`) are skipped entirely. |
...
| `ref-version-mismatch` | [`zizmor:ref-version-mismatch`](https://docs.zizmor.sh/audits/#ref-version-mismatch) | warning | Not planned | Needs live GitHub API resolution of tag → SHA, which the edge linter does not perform. **Delegated to the companion action** (`companion-action/`): karinto surfaces SHA-pinned refs (with their trailing `# vN` comment) in `online_audit_candidates`, and the companion compares against the tag and reports directly. See [docs/action-context.md](action-context.md). |
...
| `template-injection` | [`zizmor:template-injection`](https://docs.zizmor.sh/audits/#template-injection) | error | Implemented | Scans every `run:` and every known sink-action input (`actions/github-script.script`, `amadevus/pwsh-script.script`, `jannekem/run-python-script-action.script`, `cardinalby/js-eval-action.expression`, `addnab/docker-run-action.{options,run}`, `azure/cli.inlineScript`, `azure/powershell.inlineScript`, `mikefarah/yq.cmd`, `tibdex/backport.{title_template,body_template}`). Promotes to `error` when the interpolation references a high-cap context (PR/issue/comment/review/discussion body or title, head_ref, head_commit.message, workflow_run head fields, etc.); otherwise fires the upstream Info backstop on every `${{ … }}`. Subsumes the actionlint-side `script-injection` check (catalogued separately as Not planned). Known divergences: zizmor scopes `# zizmor: ignore[…]` per node while karinto's parser is file-wide, and zizmor consults a local `zizmor.yml` config that karinto doesn't read — both tracked via the parity allowlist. |
| `undocumented-permissions` | [`zizmor:undocumented-permissions`](https://docs.zizmor.sh/audits/#undocumented-permissions) | info | Implemented | Re-scans raw source to honour comments; skips `contents: read`-only blocks (including YAML-quoted forms `contents: "read"` / `contents: 'read'`). |
...
| `unpinned-tools` | [`zizmor:unpinned-tools`](https://docs.zizmor.sh/audits/#unpinned-tools) | warning | Implemented | Detects `curl|wget | sh` patterns plus tool-installer actions (`aquasecurity/setup-trivy`, `1password/load-secrets-action`) whose `with.version` is implicit, `latest`, or expression-based. Steps whose `if:` is statically false (`if: false` or `if: ${{ false }}`) are skipped entirely. Upstream's tool-installer catalogue is broader. `#113`: the `curl|wget | sh` half is now tree-sitter-bash `pipeline`-structure-based instead of `contains("curl ") && contains("| sh")`, which fired on script text merely mentioning both substrings anywhere (any order, any statement). |
...
| `unredacted-secrets` | [`zizmor:unredacted-secrets`](https://docs.zizmor.sh/audits/#unredacted-secrets) | error | Implemented | Scans `run:` and `env:` for `base64`/`xxd`/`tr`/`cut`/`sed`/`awk` pipes and `fromJSON(secrets…)` expressions that bypass redaction. `#113`: the `run:` pipe-transform half is now tree-sitter-bash `pipeline`-structure-based (real piped-command detection, not `"| base64"` anywhere in the script text); the `env:` half and the `fromJSON(secrets…)` check (neither is shell syntax) are unchanged. |
...
## karinto-original family

No upstream defines these — they're built on the tree-sitter-bash
integration added for the rules above (`#113`). Deliberately narrower in
scope than stock shellcheck; see the `shellcheck` entry's notes for why
karinto doesn't attempt shellcheck-equivalent coverage.
...
| `shell-undefined-var` | `karinto:shell-undefined-var` | info | Implemented | SC2154-like: flags a `run:` variable reference with no visible source — not declared in `env:` at workflow, job, or step level, not assigned earlier in the same script (`FOO=bar`), and not a well-known runner/shell built-in (`PATH`, `GITHUB_*`, `RUNNER_*`, `ACTIONS_*`, `INPUT_*`, …). Scoped to what the syntax tree makes reliable — no modelling of conditionals, functions, or `source`d files — hence `info`, not a harder severity. |
