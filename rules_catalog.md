...
| `config-variables` | [`actionlint:config-variables`](https://github.com/rhysd/actionlint/blob/main/docs/config.md) | error | Implemented | Dormant by default. Activated by a caller-supplied `config-variables` allowlist (native `karinto.yaml`, or a compatible `.github/actionlint.yaml` — same key/shape as upstream); flags `vars.<name>` references not in the list. See [#112](https://github.com/toiroakr/karinto/issues/112). |
