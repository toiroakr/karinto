...
| `archived-uses` | [`zizmor:archived-uses`](https://docs.zizmor.sh/audits/#archived-uses) | warning | Implemented | Three additive sources: the engine's hardcoded baseline (`actions/setup-ruby`, fires offline), a KV-published baseline that `cf/index.js`'s daily cron maintains (the request path enqueues seen `uses:` repos into a D1 worklist; the cron confirms each via the GitHub API `archived` flag and re-verifies a rotating slice weekly), and the per-request `archived` parameter CSV. Comparison is case-insensitive and operates on the bare `owner/repo` (subpath stripped). |
