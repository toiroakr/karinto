...
| `expression-type-mismatch` | [`actionlint:expression-type-mismatch`](https://github.com/rhysd/actionlint/blob/main/docs/checks.md) | error | Implemented | Literal-level inference: parses every `${{ … }}` and flags arithmetic operators (`+`/`*`/`/`/`%` — none exist in the expression language) applied to known non-number operand types (literals and known function returns). Context-derived operand types stay with the per-context rules below; upstream additionally types every context property and function signature. |
