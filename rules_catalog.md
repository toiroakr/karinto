...
| `archived-uses` | [`zizmor:archived-uses`](https://docs.zizmor.sh/audits/#archived-uses) | warning | Implemented | Mirrors zizmor's hardcoded offline baseline (`actions/setup-ruby`) so the rule fires without network access; embedders of `lint_string` can extend it via the `archived_uses_str` CSV (refs whose repo GitHub reports as `archived: true`). HTTP routing for this input is not yet wired in `cf/index.js`. Comparison is case-insensitive and operates on the bare `owner/repo` (subpath stripped). |
...
| `forbidden-uses` | [`zizmor:forbidden-uses`](https://docs.zizmor.sh/audits/#forbidden-uses) | warning | Implemented | Denylist threaded via the `forbidden_uses_str` CSV on the `lint_string` boundary; HTTP routing is not yet wired in `cf/index.js`. Each entry is a glob matched against both the bare `owner/repo[/subpath]` and the full `…@ref`. Empty list keeps the rule dormant. |
