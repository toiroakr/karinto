...
| `archived-uses` | [`zizmor:archived-uses`](https://docs.zizmor.sh/audits/#archived-uses) | warning | Implemented | Three additive sources: the engine's hardcoded baseline (`actions/setup-ruby`, fires offline), a KV-published baseline maintained by a daily CI job (the request path enqueues seen `uses:` repos into a D1 worklist; `refresh-archived.yml` drains it, confirms each via the GitHub API `archived` flag, and re-verifies the existing set on every run), and the per-request `archived` parameter CSV. Comparison is case-insensitive and operates on the bare `owner/repo` (subpath stripped). |
