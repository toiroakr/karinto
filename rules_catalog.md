...
| `archived-uses` | [`zizmor:archived-uses`](https://docs.zizmor.sh/audits/#archived-uses) | warning | Implemented | Mirrors zizmor's hardcoded offline baseline (`actions/setup-ruby`) so the rule fires without network access; the caller (an action runner) resolves which refs GitHub reports as `archived: true` and supplies them as the `archived` request parameter CSV (wired through `cf/index.js` to `lint_string`'s `archived_uses_str`). Comparison is case-insensitive and operates on the bare `owner/repo` (subpath stripped). |
...
| `impostor-commit` | [`zizmor:impostor-commit`](https://docs.zizmor.sh/audits/#impostor-commit) | error | Implemented | Offline-silent (mirrors zizmor `--no-online-audits`); the caller (an action runner) resolves SHA membership via the GitHub API and supplies the confirmed-impostor `owner/repo@sha` list as the `impostor` request parameter CSV (wired through `cf/index.js` to `lint_string`'s `impostor_uses_str`). |
...
| `ref-version-mismatch` | [`zizmor:ref-version-mismatch`](https://docs.zizmor.sh/audits/#ref-version-mismatch) | warning | Implemented | Offline-silent (mirrors zizmor `--no-online-audits`); the caller (an action runner) resolves tag → SHA via the GitHub API and supplies the confirmed-mismatch `owner/repo@sha` list as the `ref_mismatches` request parameter CSV (wired through `cf/index.js` to `lint_string`'s `ref_mismatches_str`). |
